...
# Cложите получившиеся числа и верните из функции 
# в качестве ответа вместе с преобразованным списком





languages = list (map(lambda language:language.upper(),['fortran', 'Python', 'pascal',  'assembler', 'Java', 'C++', 'C#']))

print(f"\n{languages}")
number_for_language = [num for num in range(1, len(languages)+1)]

numbers_languages = list(zip(number_for_language,languages))
print(f"\n{numbers_languages}")

def convert_numbers_of_languages(numbers_and_languages):
    converted_list = []
    sum_of_ord = 0
    for numbers,languages in numbers_and_languages:
        for name in languages:
            sum_of_ord+=ord(name)
        if sum_of_ord % numbers == 0:
            converted_list.append((sum_of_ord,languages))
    return converted_list  

print(f"\n{convert_numbers_of_languages(numbers_languages)}\n") 